...
bias = 600 # edges search range, center-bias -> center+bias
tube_width = 19.10
text_bias = 350
space = 50
...
def canny_edge_detection(frame, bias = 600, tube_width = 19.10): 
    ...
    return blurred, edges, abs_dist
prev_frame_time = 0
new_frame_time = 0
while True:
    new_frame_time = time.time() 
    ret, frame = cap.read()
    blurred, edges, abs_dist = canny_edge_detection(frame, bias, tube_width) 
    ...
    for i in range(0+space, edges.shape[0], space):
        ...
        cv2.putText(edges, str(f"{abs_dist[i]:.2f}")+"mm", (r+14, i+14), font, 1.3, (255, 255, 255), 4, cv2.LINE_AA) # 在横线上显示数字
    edges = cv2.resize(edges , (918 , 768))
    frame = cv2.resize(frame , (918 , 768))
